        ...
        <TabControl HorizontalAlignment="Left" Height="730" Margin="10,0,0,0" VerticalAlignment="Top" Width="1260">
            ...
            <TabItem Name="TabItemDeployWU" Header="Deploy Windows Updates" Margin="364,0,-398,0">
                <Grid Background="#FFE5E5E5">
                    ...
                    <TextBox Name="TextBoxWuServerIpAddress"  HorizontalAlignment="Left" Height="22" Margin="180,32,0,0" Text="192.168.13.101" VerticalAlignment="Top" Width="180" TabIndex="1" IsTabStop="False" RenderTransformOrigin="0.673,0.523"/>
                    ...
                    <TextBox Name="TextBoxWuAdminPassword" HorizontalAlignment="Left" Height="22" Margin="180,98,0,0" Text="*********" VerticalAlignment="Top" Width="180"/>
                    <DatePicker  Name="DateTimePickerWU" HorizontalAlignment="Left" Height="25" Margin="180,130,0,0" VerticalAlignment="Top" Width="250"/>
...
# Init Windows Presentation Form Functions
	...
	Function DeployRdsStart {
		Param($syncHash,$RdsServerIpAddress,$RdsServerName,$AdminUserName,$AdminPassword,$CheckBoxOstFolderRootPath,$OstFolderRootPath,$CheckBoxUserDataFolderRootPath,$UserDataFolderRootPath,$RdsOuPath,$DomainDnsName,$CheckBoxRas,$RasLicenseEmail,$RasLicensePassword,$CheckBoxRasKey,$RasKey)
        $Runspace = [runspacefactory]::CreateRunspace()
        $Runspace.ApartmentState = "STA"
        $Runspace.ThreadOptions = "ReuseThread"
        $Runspace.Open()
            ...
            While ( $job.State -eq 'Running' ) { Start-Sleep -Milliseconds 1500 ; $I += 2 ; If ( $I -ge 100 ) { $I = 1 }; $syncHash.Window.Dispatcher.invoke( [action]{ $syncHash.ProgressBarProgress3.Value = $I } ) }
             ...
             If ( $CheckBoxRas ) {
			    ...
			    $Job = Invoke-Command -ComputerName "$RdsServerIpAddress"  -Credential $Credential  -AsJob -JobName "Deploy Parallels RAS Farm" -ScriptBlock {
                    ...
                    New-RASSession -Username $AdminUserName -Password $SecPassword -Server $RdsServerIpAddress
                    ...
                    Invoke-Apply
                    New-RDS -Server $RdsServerName -NoInstall -NoRestart
                    Invoke-Apply
...
"@
	...
	Function DeployWuStart {
		Param($syncHash,$WuServerIpAddress,$WuAdminUserName,$WuAdminPassword)
        $Runspace = [runspacefactory]::CreateRunspace()
        $Runspace.ApartmentState = "STA"
        $Runspace.ThreadOptions = "ReuseThread"
        $Runspace.Open()
        ...
        $code8 = {
			...
			$Job = Invoke-Command -ComputerName "$WuServerIpAddress"  -Credential $Credential  -AsJob -JobName 'Assemble c:\windows\WindowsUpdate.ps1' -ScriptBlock {
            $Script = @'
...
'@
            ...
            While ( $job.State -eq 'Running' ) { Start-Sleep -Milliseconds 1500 ; $I += 2 ; If ( $I -ge 100 ) { $I = 1 }; $syncHash.Window.Dispatcher.invoke( [action]{ $syncHash.ProgressBarProgress8.Value = $I } ) }
			...
			$Job = Invoke-Command -ComputerName "$WuServerIpAddress"  -Credential $Credential  -AsJob -JobName 'Creating Scheduled Job CMscripts\WindowsUpdate' -ScriptBlock {
...
# Init Windows Presentation Form Actions
    ...
    $syncHash.DeployWUStart.Add_Click({
		...
		DeployWUStart -syncHash $syncHash -WuServerIpAddress $syncHash.TextBoxWuServerIpAddress.Text -WuAdminUserName $syncHash.TextBoxWuAdminUserName.Text -WuAdminPassword $syncHash.TextBoxWuAdminPassword.Text
        })
